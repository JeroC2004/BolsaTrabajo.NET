Integrantes:
- Integrante 1:
                Nombre: Jerónimo Corvi
                Legajo: 52234
                Mail: corvijeronimo@gmail.com

- Integrante 2: Nombre: Elías Moramarco
                Legajo: 53413
                Mail: eliasmoramarco@hotmail.com.ar

- Integrante 3: Nombre: Francisco Julián Coux
                Legajo: 50749
                Mail: franciscocoux0@gmail.com



Descripción del Sistema: 
1. Introducción:
El proyecto tiene como objetivo presentar una plataforma de vinculación laboral orientada a estudiantes y empresas. La propuesta busca ofrecer un espacio funcional para la publicación de ofertas laborales, la gestión de perfiles y la interacción entre los distintos actores involucrados.

2. Objetivo del Proyecto:
Desarrollar una aplicación que facilite el acceso de los alumnos a pasantías y primeros empleos, centralizando la demanda de las empresas y la oferta de los estudiantes de manera eficiente.

3. Alcance y Funcionalidades:
El sistema se centrará en un modelo de dos roles principales: Oferta (Empresas/Reclutadores) y Demanda (Alumnos).
Funcionalidades clave:
- Gestión de Usuarios: Registro y validación de alumnos.
- Portal de Empresas: Creación de perfiles corporativos y publicación de vacantes.
- Panel de Alumnos: Carga de CV digital y filtrado de búsquedas por área de interés.
- Sistema de Notificaciones: Avisos sobre nuevas ofertas que coincidan con el perfil del alumno.

4. Casos de Uso Principales:
   
CU 1: Publicación de Oferta Laboral
Actor: Empresa.
Descripción: El representante de la empresa ingresa al sistema, completa un formulario con los detalles del puesto y publica la oferta para que sea visible por los alumnos.

CU 2: Postulación a Oferta
Actor: Alumno.
Descripción: El alumno navega por el listado de ofertas activas, selecciona una de su interés y presiona el botón "Postularse". El sistema vincula automáticamente su perfil/CV con la oferta seleccionada.

CU 3: Gestión de Perfil de Alumno
Actor: Alumno.
Descripción: El alumno carga sus datos personales, académicos y adjunta su CV en formato PDF. Estos datos serán los que reciba la empresa al momento de una postulación.

CU 4: Visualización de Postulantes
Actor: Empresa.
Descripción: La empresa accede a un panel donde puede ver la lista de alumnos que se postularon a sus búsquedas y descargar sus currículums.

5. Modelo de dominio
https://drive.google.com/file/d/1PjSg7Oh3x5IxQZ8GpnKMqPqqBYt8h-qX/view?usp=sharing

